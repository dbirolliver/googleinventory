...
const ICONS = {
    ...
    'Inventory History': <svg xmlns="http://www.w3.org/2000/svg" className="h-6 w-6" viewBox="0 0 20 20" fill="currentColor"><path fillRule="evenodd" d="M4 4a2 2 0 012-2h4.586A2 2 0 0112 2.586L15.414 6A2 2 0 0116 7.414V16a2 2 0 01-2 2H6a2 2 0 01-2-2V4zm2 6a1 1 0 011-1h6a1 1 0 110 2H7a1 1 0 01-1-1zm1 3a1 1 0 100 2h6a1 1 0 100-2H7z" clipRule="evenodd" /></svg>,
    'Clinic Admin': <svg xmlns="http://www.w3.org/2000/svg" className="h-6 w-6" viewBox="0 0 20 20" fill="currentColor"><path d="M10.707 2.293a1 1 0 00-1.414 0l-7 7a1 1 0 001.414 1.414L4 10.414V17a1 1 0 001 1h2a1 1 0 001-1v-2a1 1 0 011-1h2a1 1 0 011 1v2a1 1 0 001 1h2a1 1 0 001-1v-6.586l.293.293a1 1 0 001.414-1.414l-7-7z" /><path d="M10.707 2.293a1 1 0 00-1.414 0l-7 7a1 1 0 001.414 1.414L4 10.414V17a1 1 0 001 1h2a1 1 0 001-1v-2a1 1 0 011-1h2a1 1 0 011 1v2a1 1 0 001 1h2a1 1 0 001-1v-6.586l.293.293a1 1 0 001.414-1.414l-7-7z" /></svg>,
...
const Sidebar: React.FC<SidebarProps> = ({ currentUser, onLogout, currentPage, setCurrentPage, isMobileOpen, setMobileOpen }) => {
    ...
    const NAV_ITEMS = ['Dashboard', 'Supplies'];
    const ADMIN_NAV_ITEMS = ['Suppliers', 'Supplier Insights', 'Clinic Admin', 'User Admin'];
    ...
    <aside className={`fixed top-0 left-0 h-screen bg-gray-950/70 backdrop-blur-lg p-4 flex flex-col justify-between border-r border-white/10 z-40
                        transform transition-all duration-300 ease-in-out group hover:w-64 lg:w-20
      ...
      <div>
        <div className="flex items-center space-x-3 mb-10 h-12 px-2">
            ...
            <h1 className="text-xl font-bold text-white truncate transition-opacity opacity-0 group-hover:opacity-100 delay-200">Premierlux</h1>
        </div>
        ...
        <nav className="space-y-2">
            ...
            {!isAdmin && (
                <NavItem 
                    ...
                    icon={ICONS['Inventory History']} 
                    isActive={currentPage === 'Inventory History'} 
                    onClick={() => handleNavClick('Inventory History')} />
            )}
