...
const BranchAdmin: React.FC<BranchAdminProps> = ({ performanceData, onAddBranch, onEditBranch, onDeleteBranch }) => {
  ...
  const handleEditSubmit = (e: React.FormEvent) => {
    ...
    onEditBranch(editingBranch.id, editingBranch.name);
  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
      <div className="md:col-span-2 bg-white/10 backdrop-blur-lg p-6 rounded-xl border border-white/20">
        <h2 className="text-xl font-semibold text-gray-200 mb-4">Branch Performance Overview</h2>
        <div className="overflow-x-auto">
          <table className="w-full text-left min-w-[640px]">
            <thead className="border-b-2 border-white/30">
              <tr>
                <th className="py-3 px-4 text-sm font-semibold text-gray-300 uppercase">Branch Name</th>
              ...
              {performanceData.map(branch => (
                <tr key={branch.branchId} className="border-b border-white/10">
                  ...
                  <td className="py-4 px-4 text-center">
                    <button onClick={() => setEditingBranch({ id: branch.branchId, name: branch.branchName })} className="text-blue-400 hover:text-blue-300 text-sm font-medium">Edit</button>
                    <button onClick={() => onDeleteBranch(branch.branchId)} className="text-red-400 hover:text-red-300 text-sm font-medium ml-4">Delete</button>
                  </td>
                </tr>
              ))}
            </tbody>
      ...
      <div className="bg-white/10 backdrop-blur-lg p-6 rounded-xl border border-white/20">
        {editingBranch ? (
          <>
            <h2 className="text-xl font-semibold text-gray-200 mb-4">Edit Branch</h2>
            <form onSubmit={handleEditSubmit} className="space-y-4">
              <div>
                <label htmlFor="editBranchName" className="block text-sm font-medium text-gray-300 mb-1">Branch Name</label>
                <input
                  id="editBranchName"
                  type="text"
              ...
              <div className="flex space-x-2">
                <button type="button" onClick={() => setEditingBranch(null)} className="w-full bg-white/10 text-white font-bold py-3 rounded-lg border border-white/30 hover:bg-white/20 transition">Cancel</button>
          ...
          <>
            <h2 className="text-xl font-semibold text-gray-200 mb-4">Add New Branch</h2>
            <form onSubmit={handleAddSubmit} className="space-y-4">
              <div>
                <label htmlFor="newBranchName" className="block text-sm font-medium text-gray-300 mb-1">Branch Name</label>
                <input
                  id="newBranchName"
                  type="text"
                  value={newBranchName}
