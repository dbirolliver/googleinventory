...
const SuggestionCards: React.FC<SuggestionCardsProps> = ({ suggestions, branches, onAcknowledge }) => {
        ...
        {relevantSuggestions.map(item => (
          <div key={item.id} className="bg-black/20 p-4 rounded-lg border border-white/20 flex flex-col justify-between">
            <div>
              ...
              <p className="text-sm text-gray-400">Predicted Sales (30d): <strong className="text-gray-200">{item.predictedSales?.toLocaleString() ?? 'N/A'}</strong></p>
            ...
            <button
              ...
              className="w-full text-center mt-4 bg-white/10 text-white text-sm font-bold py-2 px-4 rounded-lg border border-white/30 hover:bg-white/20 focus:outline-none focus:ring-4 focus:ring-white/30 transition-all duration-300"
            >
              Acknowledge
            </button>
          </div>
