...
const EditProductModal: React.FC<EditProductModalProps> = ({ product, suppliers, branches, onClose, onSave }) => {
  ...
  return (
    <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex justify-center items-center z-50" onClick={onClose}>
      <div className="bg-gray-800/50 backdrop-blur-xl border border-white/20 rounded-xl shadow-lg p-8 m-4 max-w-lg w-full max-h-[90vh] overflow-y-auto" onClick={e => e.stopPropagation()}>
        <h2 className="text-2xl font-bold text-white mb-6">Edit Product Details</h2>
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div>
            <label htmlFor="minStockLevel" className="block text-sm font-medium text-gray-300 mb-1">Minimum Stock Level (Total)</label>
            ...
            <p className="text-xs text-gray-400 mt-1">Set a threshold for high-urgency alerts across all clinics.</p>
          </div>
